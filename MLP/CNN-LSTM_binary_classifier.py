...
OK_csv = ["OK1_FILTERED_w{}", "OK2_FILTERED_w{}", "OK3_FILTERED_w{}", "OK4_FILTERED_w{}"]
STAND_csv = ["STANDING1_FILTERED_w{}", "STANDING2_FILTERED_w{}"]
...
OK_csv = ["OK1_2_w{}_norm_{}", "OK3_4_w{}_norm_{}"]
STAND_csv = ["STANDING1_w{}_norm_{}", "STANDING2_w{}_norm_{}"]
...
# Modelling
# Standard Keras model save and load
...
def show_graph(file='training.log'):
    ...
    dic = {'loss': ['loss', 'val_loss'], 'acc': ['acc', 'val_acc'],
...
"""
A custom DataGenerator for generating data during the training phase.
Impossible to load and shape all the entire examples on RAM.
"""
class DataGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        x = np.array(px)
        x = x.reshape((x.shape[0], 5, 5000, len(self.seq_cols)))  # the only difference from the LSTM_binary_classifier
        # 5 (the # of TimeDistr-Conv1) * 5000 = 25000 one signal length
        y = np.array(py)
        return x, y


# Creation of the CNN-LSTM model
def create_CNN_LSTM_model(b_s, state=False):
    model = Sequential()
    model.add(TimeDistributed(Conv1D(filters=64, kernel_size=3, activation='relu'),
                              input_shape=(None, 5000, len(sequence_cols))))
    model.add(TimeDistributed(Conv1D(filters=64, kernel_size=3, activation='relu')))
...
def load_model_hdf5(file):
    model = create_CNN_LSTM_model(batch_size)
...
def model_generation(sequence_length, batch_size, tdg, vdg, ne, state=False, optimizer=None):
    ...
    model = create_CNN_LSTM_model(batch_size, state)

    model.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy'])

    model.summary()
    model.fit_generator(generator=tdg, validation_data=vdg, epochs=ne,
                        verbose=1, callbacks=[csv_logger, rms, mck, es], shuffle=False)
...
def evaluate_model(model, gen):
    scores = model.evaluate_generator(gen, verbose=1)
    print('Accurracy: {}'.format(scores[1]))


def predict_model(model, gen, title):
    # make predictions and compute confusion matrix
    y_pred_prob = model.predict_generator(gen, verbose=1)
...
if __name__ == '__main__':
    ...
    # 1 - Load model file
    # model = load_model("CNN-LSTMmodel")
    # OR 2 Load from hdf5
    ...
    model.compile(loss='binary_crossentropy', optimizer=optimizer, metrics=['accuracy'])
    model.summary()
    show_graph(file="CNN-LSTMtraining.log")
    """
